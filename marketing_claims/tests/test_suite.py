...
Run with: python -m unittest marketing_claims.tests.test_suite
Or from repo root: python -m pytest marketing_claims/tests/
"""
...
class TestEnrichmentClient(unittest.TestCase):
    ...
    @patch("marketing_claims.enrichment_client.urllib.request.urlopen")
...
class TestFlaskApp(unittest.TestCase):
    ...
    @patch("marketing_claims.app.evaluate_with_anthropic")
    @patch("marketing_claims.app.enrich_product")
    def test_evaluate_malformed_custom_prompt(self, mock_enrich, mock_llm):
        """Test that malformed custom prompt templates return graceful error."""
        ...
        resp = self.client.post("/api/evaluate", json={
            "products": [{
                "upc": "041100001214",
                "product_name": "Coppertone",
                "marketing_claims": "Broad spectrum.",
            }],
            "llm_provider": "anthropic",
            "llm_api_key": "test-key",
            "evaluation_prompt": "Invalid {unknown_placeholder} template",
        })
        ...
        self.assertIn("Invalid custom prompt template", result["llm_error"])
        self.assertIn("Allowed placeholders", result["llm_error"])
    ...
    @patch("marketing_claims.app.evaluate_with_gemini")
    @patch("marketing_claims.app.enrich_product")
