...
const App = {
    ...
    renderDetailPanel(result) {
        ...
        // Safe class mappings for verdict and severity values
        const safeVerdictClasses = {
            'VALID': 'valid',
            'INVALID': 'invalid',
        ...
        claims.forEach(claim => {
            const verdictKey = String(claim.verdict || '').toUpperCase();
            const severityKey = String(claim.severity || '').toUpperCase();
            const vclass = safeVerdictClasses[verdictKey] || '';
            const sclass = safeSeverityClasses[severityKey] || '';
            const badgeClass = safeBadgeClasses[verdictKey] || 'badge-partial';
            html += `
                <div class="claim-item ${vclass}">
                    <div class="claim-text">
                        <span class="badge ${badgeClass}">${this.escapeHtml(claim.verdict)}</span>
                        <span class="${sclass}" style="margin-left:8px;font-size:11px">${this.escapeHtml(claim.severity)}</span>
